...
namespace Pinwheel.Griffin.StampTool
{
    ...
    public class GObjectStamperInspector : Editor
    {
        ...
        private class GLayerGUI
        {
            ...
            public static readonly string HEADER_SPLAT_WEIGHT_RULE = "Splat Weight Rule";
            public static readonly GUIContent BLEND_SPLAT_WEIGHT = new GUIContent("Enable", "Toggle terrain splat weight rule");
        }
        ...
        private void DrawLayer(GObjectStampLayer layer, int index)
        {
            ...
            GEditorCommon.Foldout(label, false, id, () =>
            {
                ...
                GEditorCommon.Header(GLayerGUI.HEADER_SPLAT_WEIGHT_RULE);
                layer.BlendSplatWeight = EditorGUILayout.Toggle(GLayerGUI.BLEND_SPLAT_WEIGHT, layer.BlendSplatWeight);
                if (layer.BlendSplatWeight)
                {
                    layer.SplatWeightIndex = GEditorCommon.SplatSetSelectionGrid(instance.GroupId, layer.SplatWeightIndex);
                    EditorGUI.BeginChangeCheck();
                    layer.SplatWeightRemap = EditorGUILayout.CurveField(GLayerGUI.HEIGHT_TRANSITION, layer.SplatWeightRemap, Color.red, new Rect(0, 0, 1, 1));
                    if (EditorGUI.EndChangeCheck())
                    {
                        layer.UpdateCurveTextures();
                    }
