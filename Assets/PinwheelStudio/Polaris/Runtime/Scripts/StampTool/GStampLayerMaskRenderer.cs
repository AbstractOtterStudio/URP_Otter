...
namespace Pinwheel.Griffin.StampTool
{
    public static class GStampLayerMaskRenderer
    {
        ...
        private const string BLEND_SPLAT_WEIGHT_KW = "BLEND_SPLAT_WEIGHT";
            ...
            bool useTerrainMask)
        {
            ...
            float minHeight = GUtilities.InverseLerpUnclamped(0, terrainSize.y, layer.MinHeight);
            float maxHeight = GUtilities.InverseLerpUnclamped(0, terrainSize.y, layer.MaxHeight);
            brushMat.SetFloat("_MinHeight", minHeight);
            brushMat.SetFloat("_MaxHeight", maxHeight);
            ...
            if (layer is GConditionalStampLayer.IHasSplatWeightRule splatWeightRuleLayer)
            {
                int splatControlIndex = splatWeightRuleLayer.SplatWeightIndex / 4;
                bool validControlIndex = splatControlIndex >= 0 && splatControlIndex < terrain.TerrainData.Shading.SplatControlMapCount;
                if (validControlIndex)
                {
                    Texture2D controlMap = terrain.TerrainData.Shading.GetSplatControlOrDefault(splatControlIndex);
                    brushMat.SetTexture("_SplatControlMap", controlMap);
                    brushMat.SetTexture("_SplatControlRemap", splatWeightRuleLayer.GetSplatWeightRemapTexture());
                    brushMat.SetFloat("_SplatControlChannel", splatWeightRuleLayer.SplatWeightIndex % 4);
                }

                GCommon.SetMaterialKeywordActive(brushMat, BLEND_SPLAT_WEIGHT_KW, splatWeightRuleLayer.BlendSplatWeight && validControlIndex);
